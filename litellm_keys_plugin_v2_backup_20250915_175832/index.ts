...
/**
 * Get the status of a LiteLLM key
 */
export const getLiteLLMKeyStatus = async (token: string, keyId: string): Promise<any> => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/litellm-keys/${keyId}/status`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
 ...
 */
export const testLiteLLMConnection = async (token: string): Promise<any> => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/litellm-keys/health/connection`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
